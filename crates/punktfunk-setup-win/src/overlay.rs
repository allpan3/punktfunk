//! S2 spike: the `[exe][compressed payload][footer]` sandwich (design D3).
//!
...
//! directory's file offset as the true end of the overlay (EOF when unsigned).
//!
...
pub fn cert_table_offset(data: &[u8]) -> Option<u32> {
    ...
    Some(u32::from_le_bytes(data.get(entry..entry + 4)?.try_into().ok()?))
}
...
mod tests {
    ...
    fn fake_pe(cert_offset: u32, tail: usize) -> Vec<u8> {
