...
| `PUNKTFUNK_ABR_LADDER` | `0` | Keep the picture at the size and refresh rate you picked, however thin the link gets. Normally, when the adaptive bitrate falls too low to feed the negotiated mode, the client steps down a rung — 1080p30 to 1080p20 to 720p30 to 720p20 — because a smaller picture at the same bitrate has bits per pixel to spare, and it climbs back once the rate recovers. With this set the mode never moves, and a starved session stays at your size and goes blocky instead. |
