...
| `PYROWAVE_QUEUE_PRIORITY` | `realtime` (default) · `high` · `off` | [PyroWave](/docs/pyrowave): ask the GPU to run the encode ahead of the game. On Linux only `punktfunk-encode-worker` may raise it, never the host — see [GPU scheduling priority](/docs/running-as-a-service#gpu-scheduling-priority). `off` if the desktop stutters while streaming. |
