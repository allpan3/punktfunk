...
|---|---|---|---|
| Wire protocol | 2 | **2** | unchanged; optional tagged extension block (`PUNKTFUNK_CLIENT_CAP_EXT` on Hello, `PUNKTFUNK_HOST_CAP2_EXT` on Welcome), startup-ramp and delivery-report capability bits, message ids 89 `AUDIO_STATE`, 90 `LAUNCH_OUTCOME`, 91 `PAD_SLOTS`, input kind 16 `SCROLL` with `PunktfunkScroll{Source,Phase}`, a client label on `Start` — all opt-in, so an older peer's handshake is byte-identical |
...
| `api/openapi.json` | 0.38.0 | **0.39.0** | `GET/PATCH /host/settings`, `/host/audio/apps`, `/plugin-access{,/requests,/{plugin}/decide}`, `/session/{id}` with `audio`/`idr`/`pads`/`player`/`access`, `/session/last`, `GET /library/custom/{id}`, `/game/end` takes an id |
...
| `@punktfunk/plugin-kit` | 0.4.6 | **0.5.2** | Sandbox contract: manifest required, per-plugin token, reviewed read-only folder grants (a plugin may also request mid-flight), ACL re-read on respawn, `reg.exe` respawn |
...
- `PUNKTFUNK_ABR_PROBE=0` skips the startup link measurement, `PUNKTFUNK_ABR_PROBE_KBPS`
  bounds what it asks, `PUNKTFUNK_ABR_MAX_MBPS` caps the Automatic climb regardless of it,
  `PUNKTFUNK_PYROWAVE_MAX_MBPS` caps a PyroWave pin (registry row `pyrowave_max_mbps`).
- `punktfunk1-host --source synthetic-abr` sizes each frame from the session's running
  rate — a headless rig for measuring Automatic; `--content`, `--fill`, `--recovery-ms`
  and `--keyframe-answer` shape it.
...
- A custom library entry's `audio.sessions` (`all` · `owner` · `joined` · `launcher`) picks who
  hears the title.
- Wire constants, not knobs: `PUNKTFUNK_EXT_*` bounds and ABR tags,
  `PUNKTFUNK_HOST_CAP2_{RAMP,DELIVERY}`, `PUNKTFUNK_MSG_*` ids,
