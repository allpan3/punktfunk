...
export type PluginRegistration = { readonly "category"?: string | null, readonly "title": string, readonly "ui"?: null | { readonly "config"?: boolean, readonly "game"?: boolean, readonly "icon"?: string | null, readonly "page"?: boolean, readonly "port": number, readonly "secret": string }, readonly "version"?: string | null }
export const PluginRegistration = Schema.Struct({ "category": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null]).annotate({ "description": "Plugin kind, not a UI field. Console hides `library` from the nav (those plugins already have\nGame sources); omit the field to keep a nav page." })), "title": Schema.String, "ui": Schema.optionalKey(Schema.Union([Schema.Null, Schema.Struct({ "config": Schema.optionalKey(Schema.Boolean.annotate({ "description": "Serves `GET/PUT /__config`, the plugin-wide settings form." })), "game": Schema.optionalKey(Schema.Boolean.annotate({ "description": "Serves `GET/PUT /__game?entry=<id>`, a tab on each library entry's page." })), "icon": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null])), "page": Schema.optionalKey(Schema.Boolean.annotate({ "description": "Serves a page the console opens and lists in the nav. Absent means yes, as before the flag." })), "port": Schema.Number.annotate({ "description": "Loopback only — the host dials `127.0.0.1:<port>`; a registration cannot carry a hostname.", "format": "int32" }).check(Schema.isInt()).check(Schema.isGreaterThanOrEqualTo(0)), "secret": Schema.String.annotate({ "description": "Per-boot; the console proxy presents it as `Authorization: Bearer`. Rotated on plugin restart." }) }).annotate({ "description": "Absent `ui` is a live listing with no nav entry — not an error." })], { mode: "oneOf" })), "version": Schema.optionalKey(Schema.Union([Schema.String, Schema.Null])) })
...
export type HookEntry = { readonly "debounce_ms"?: number, readonly "filter"?: null | { readonly "app"?: string | null, readonly "client"?: string | null, readonly "fingerprint"?: string | null, readonly "plane"?: null | Plane, readonly "preset"?: string | null }, readonly "hmac_secret_file"?: string | null, readonly "on": string, readonly "run"?: string | null, readonly "timeout_s"?: number, readonly "webhook"?: string | null }
